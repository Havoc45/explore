...
| `--reconcile` | Refresh `docs/system-design-reference/` and verify/relink `plans/` against `HEAD`, + skill-usage glossary refresh (`references/closing-the-loop.md`), + housekeeping sweep (PR-merge check, repo snapshot, tracked-tab cleanup) | both · `references/system-design-reference.md`, `references/closing-the-loop.md` |
...
| `--setup-plugin` | Interactive first-run setup: detect harness + lanes, choose lanes and billing, pick the model roster, set or calibrate C/I/T weights, persist to the global config home | `~/.config/explore/roster.json` · `references/setup.md` |
...
Flags chain. `--setup-plugin` is the exception: it never chains and **bypasses the phased workflow** (no recon, no exploration — only the wizard runs); any other flags passed with it are acknowledged in one line and deferred to a re-invocation. The natural lifecycle is `[--sub-continuous] explore → [--improve / --security / --plan-once] → [--execute-level] → [--reconcile]`, modulated by `--depth`, `--verbosity`, `--caveman`, and `--model`. Worked example:
...
## First-run setup (`--setup-plugin`)

An interactive wizard that detects the host harness and the installed dispatch lanes, then asks the user to choose the lanes, their billing, the eligible models, and each model's Cost/Intelligence/Taste weights — optionally calibrating those weights against their real models.

It runs **standalone and bypasses the phased workflow entirely** — no recon, no exploration, no documentation or plan output. The answers persist as a **roster** at `${XDG_CONFIG_HOME:-$HOME/.config}/explore/roster.json`, which is machine-global user config: it survives plugin updates, serves every harness on the machine, and is never vault-routed. A valid roster is authoritative over the shipped defaults in `references/model-roster.md` — those apply when no valid roster exists **or when a valid roster leaves no locally usable lane** (`references/setup.md` "Cross-harness loading").

Read `references/setup.md` and follow it as the wizard.

## Workflow

### Phase 1 — Recon & truth-grounding (always; Hard Rule 7)

Scope the architecture and stack, then pull every source of truth: `README`, `CLAUDE.md`/`AGENTS.md`, root configs/manifests, CI, IaC, directory structure; existing design docs and ADRs (ingest as input, carry forward — a recorded tradeoff is *by design*, a stale ADR is itself a finding); **any paths passed via `--reference`** (the maintainer's own notes, specs, or glossaries — treat them as first-class ground truth, still subject to Rule 6, and cite them like any other source); the repo's own diagram/doc conventions (match them); git signal for what's evolving. Identify languages, frameworks, package manager, and the exact **build/test/lint/typecheck commands** (they become verification gates in every plan). Detect and probe which dispatch lanes the host has (`command -v codex opencode`, model availability, MCP/server health — the mandatory preflight probe in `references/delegation.md`) and note the per-lane result in the run record. Load the user roster from `${XDG_CONFIG_HOME:-$HOME/.config}/explore/roster.json` when it exists **and validates against its schema** (`references/setup.md` "Persistence") — a valid roster is authoritative over the shipped defaults (`references/model-roster.md`), which apply when no valid roster exists **or when a valid roster leaves no locally usable lane** (`references/setup.md` "Cross-harness loading"); a file that fails validation is treated as absent for the run (note it in one line). When lanes are present but no valid roster exists, suggest `--setup-plugin` in one line (a suggestion only, never a question) and continue with the shipped defaults. Also detect a **Knoxville docs-vault** (`.knoxville.json` at the repo root, a stub `CLAUDE.md` starting `<!-- knoxville-stub -->`, or a registered `knoxville` MCP server — see `references/init.md` "Knoxville handoff"): a linked vault means **every documentation output of this run** — the reference set, ADRs, plans, mirrors, the `--init` primer — routes to the vault file-for-file (the routing table in `init.md`), not the repo, and the run ends with `knoxville sync`. Use available tool calls and MCP connectors to confirm behaviour you'd otherwise guess. If `docs/system-design-reference/` or `plans/` already exists, this is a **refresh/reconcile** — read them first and update rather than clobber. Optionally seed with the bundled analyzers (read-only). If the repo has no working verification command, "establish a verification baseline" is usually plan #1. On the plugin's first invocation for a project with no recorded staffing (`references/closing-the-loop.md` "Project staffing record"), run the per-project executor/verifier picker — one structured ask, persisted to the first writable staffing slot (vault → `AGENTS.md` explore-managed block → harness memory), skipped ever after unless the roster hash invalidates it.

### Phase 1.5 — Orient (context architecture; opt-in)

When the machine config enables `optimizations.context_architecture` (`references/setup.md` "Wizard Step 5"), run the orientation pass per `references/context-architecture.md`: compose the fragment baseline broad → specific, dedup byte-identical content, bound by budget (omit broad before truncating specific, never summarize), and write the composed fragment list — relPath, why loaded, bytes — into the run record **before any Phase-2 fan-out**, so the operator can review the context that will shape the run. When the key is absent or disabled, skip this phase silently — the run behaves exactly as before.

### Phase 2 — Explore / Audit (parallel; `--depth`-bounded; `--caveman` comms)

Go deep, using the companion references: the exploration lenses from `references/architecture-patterns.md`, `references/system-design-workflows.md`, and `references/tech-decision-guide.md`; and, when an `--improve`/`--security` action is in play, the audit categories and Finding format from `references/audit-playbook.md`. The three lens references are **subagent-brief material** — the orchestrator passes section paths into worker briefs and never reads the files inline, except on no-subagent harnesses (Platform adaptation degradation). Fan out parallel read-only subagents (in Claude Code: **Explore** agents; where a provider CLI is installed, confined read-only CLI runs — the roster in `references/delegation.md`) — one per lens / category / package. Each subagent prompt must include the **absolute path** to the relevant reference section, the recon facts that scope it, an instruction to return evidence-cited observations/findings only (no fixes, no file dumps), and **a verbatim copy of Hard Rules 4 and 6**. Dispatch and monitoring follow the **org chart** ("Delegation & oversight"): brief workers with one task each, put managers over subsystems on `deep`/monorepo runs, read the heartbeats, and recall a spiraling agent up the escalation ladder rather than letting it churn. If the host can't spawn subagents (e.g. Claude.ai), work directly, lens by lens.
...
- **Two dispatch lanes.** *Native subagents* run the harness's own models (on Claude Code: Claude models — `sonnet` / `opus` / `fable`). *Provider CLIs* installed on the host (e.g. `codex` → OpenAI models, `opencode` → OpenRouter models) extend the roster across providers on **any** code-mode harness (lanes the effective roster enables — `references/delegation.md` "User roster override") — confined read-only runs for lens/audit work (codex: OS-sandboxed; opencode: permission-gated), worktree-confined runs for execution. Each provider lane is a **minion platform**: reachable over a registered MCP server (preferred — structured session ids, live heartbeats, steerable sessions) or plain shell runs, and able to spawn its own tier of native subagents under a manager brief. The default routing **reserves the session model for orchestration** — judgment, vetting, verdicts, assembly — and offloads worker-tier units to the cheapest lane that clears them, so the session quota is spent only where intelligence compounds. The routing rules live in `references/delegation.md` ("The model roster & routing"); the transports, verified call shapes, and effort mapping live in `references/delegation-transports.md`; the score table and per-model profiles in `references/model-roster.md`. State the chosen model **and effort** per plan so the run is reproducible, and **label every dispatch with its true model** — agent labels, shell-run descriptions, and worktree paths all carry it (`gpt-5.6-sol:<task>`, `[glm-5.2] <run>`, `<plan-id>-<model>/`) per the labeling rule in `references/delegation.md`; a wrapper agent shows the harness its own model, so the label is the only truth about who is working.
