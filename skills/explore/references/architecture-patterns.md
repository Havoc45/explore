...
> **How `explore` uses this.** During exploration you are *classifying a system that
> already exists*, not choosing a pattern for a greenfield build. Read this catalog to
> name the pattern the code embodies and to judge it honestly: the "When to use / When NOT
> to use" lists become a fit check ("this is a 30-developer monolith — the cons column is
> what the team is living with"), and the trade-off tables give you the language for the
> ADRs and the risk map. Classify by structural evidence (directory layout, deployment
> units, module boundaries) and state a confidence — never assert a pattern from vibes.
> Any "you should switch to X" stays a labelled option for the maintainer, never a directive.
