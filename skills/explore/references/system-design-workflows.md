<!--
  Vendored from the `senior-architect` skill (https://github.com/alirezarezvani/claude-skills),
  MIT © Alireza Rezvani. Reused here under MIT. See ../../../NOTICE.
-->

> **How `explore` uses this.** These workflows are written for designing systems forward;
> `explore` runs them *in reverse*, as analytical lenses for characterising a system that
> is already built. Capacity planning becomes "what load shape do the code and config
> imply, and where measured" (numbers only where something measures them, "needs
> measurement" elsewhere). The API and schema workflows become "document the API/schema
> as it exists and note where it diverges from these conventions." Scalability assessment
> becomes "where are the bottlenecks the architecture bakes in." Migration planning is the
> one you may genuinely use forward — when documenting a migration the system is mid-way
> through. Everything you record cites `file:line`; recommendations are labelled options.
