...
Reached from `references/setup.md` Wizard Step 7, the **calibrate** path, and only after its consent gate.

## What calibration is

A short, consent-gated **probe** run that *suggests* Cost/Intelligence/Taste values for the models the user chose. The suggestions are advisory, never verdicts — the user confirms every value back in `setup.md` Step 7 before anything is saved.
...
**Transcripts**, one per probe, are written **first to the wizard's staging directory** (`setup.md` Step 8 — nothing touches the config home before save), then moved on save to:
