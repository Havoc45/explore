<!--
  Vendored from the `improve` skill (https://github.com/shadcn/improve),
  MIT © shadcn. Reused here under MIT. See ../../../NOTICE.
-->

> **How `explore` uses this.** The category catalog and Finding format for the
> `--improve` and `--security` actions. `--security` runs section 2 only;
> `--improve` runs the relevant set. Subagent findings come back in the Finding
> format below; under `--caveman` they come back caveman-compressed (terse, but
> `file:line` evidence and error strings stay verbatim — see `caveman.md`), and
> the orchestrator expands them to `--verbosity` when writing the deliverable.
