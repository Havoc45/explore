...
> **How `explore` uses this.** Use these decision matrices to *contextualise and evaluate
> the choices the system already made*, not to pick technology for it. When the code runs
> PostgreSQL, the SQL-vs-NoSQL matrix lets you explain why that fits (or note the tension)
> given the data characteristics you observed — which is exactly the "Rationale" and
> "Trade-offs" an ADR needs. When the evidence shows a choice straining against its fit,
> the matrix grounds an *option* you surface for the maintainer to weigh. The skill
> describes and explains; it never prescribes a swap or edits anything.
