...
**Dispatch transports** — each provider lane is reachable two ways; prefer MCP where registered, shell everywhere else. **Verification is per shape, not blanket**: each shell shape carries its own pin below, and the MCP shapes are verified against the *serve* version, which can trail the CLI. Live today: codex CLI **0.145.0**; opencode CLI **1.18.6**, `opencode serve` **1.18.3**. (Shapes first verified on codex 0.142.5 / opencode 1.17.13.)
...
# (extra-roots resume shape verified on codex-cli 0.144.1; not yet re-verified on 0.145.0):
...
**Codex lane quirks** — verified through codex-cli 0.144.5, and the read-only shell shape re-verified on 0.145.0 (2026-07-27); know these before dispatching:
...
- **Stdin hang.** `codex exec` reads stdin whenever it isn't a TTY (`Reading additional input from stdin...`) — a background or harness shell leaves the pipe open and the run blocks forever *before doing anything*. Always close it: `codex exec … "<prompt>" </dev/null`. (Hit live on 0.142.5; re-tested and still present on 0.144.5. Not re-tested on 0.145.0 — every 0.145.0 dispatch carried the closure, so the hang has had no chance to show; the closure stays mandatory regardless.)
...
**opencode lane quirks** — verified on opencode 1.17.18 (2026-07-11/12 runs); lane re-verified on serve 1.18.3 (wrapper respawn + glm-5.2 xhigh MCP dispatch), most recently 2026-07-27 with `opencode_fire`/`opencode_wait` against serve **1.18.3** under CLI **1.18.6**; know these before shell dispatch:
