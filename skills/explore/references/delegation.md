...
**Dispatch transports** — each provider lane is reachable two ways; prefer MCP where registered, shell everywhere else (all shapes below live-verified, codex 0.144.5 / opencode 1.18.3; first verified on 0.142.5 / 1.17.13):
...
**Codex lane quirks** — verified through codex-cli 0.144.5; know these before dispatching:
...
- **Stdin hang.** `codex exec` reads stdin whenever it isn't a TTY (`Reading additional input from stdin...`) — a background or harness shell leaves the pipe open and the run blocks forever *before doing anything*. Always close it: `codex exec … "<prompt>" </dev/null`. (Hit live on 0.142.5; re-tested and still present on 0.144.5.)
...
**opencode lane quirks** — verified on opencode 1.17.18 (2026-07-11/12 runs); lane re-verified on 1.18.3 (wrapper respawn + glm-5.2 xhigh MCP dispatch); know these before shell dispatch:
