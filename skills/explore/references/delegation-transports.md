...
**herdr transport** — inside a herdr session (`HERDR_ENV=1`; the plugin's SessionStart hook announces it), pane agents are a third dispatch surface, preferred over bare-shell background runs for interactive agents (visible to the operator, steerable, survive the session). All herdr CLI commands return JSON. Worker dispatch: split an unfocused pane (`herdr pane split --current --direction down --ratio 0.3 --no-focus --cwd <root>` → returns the new `pane_id`), then `herdr agent start <label> --kind <roster-lane-kind> --pane <id> --timeout 60000` (kinds include `opencode`, `codex`, `claude`, `pi`, `gemini`, `cursor`, `cline`, `omp`, `kimi`, `amp`, `grok`, …) → `interactive_ready: true`; then `herdr agent prompt <label> "<brief>" --wait --until idle --until blocked`, then `herdr agent read <label>` (sources `recent` | `screen`). Executor dispatch is the same with `--cwd <worktree>` (herdr also has a `worktree` command group — unverified). Heartbeats: poll `herdr agent get/list` state (`idle` / `working` / `blocked` / `done` / `unknown`), mapped onto the 10-minute watch. Steer: `herdr agent prompt` on idle. `blocked` = a pending approval inside the pane agent — surface to the operator, never auto-answer (sole exception: the recorded Allow-always pattern on disposable panes, gotcha 3 below). Close the panes you opened (`herdr pane close <pane_id>`). Staffing/labeling rules are unchanged (the brief carries model + effort; the effective roster picks the model; `--kind` maps to the lane's CLI). Usage: prefer the `tokens` field from `herdr agent list` when the usagebar plugin is installed — observed live shape `{"context":"⛁ 52% (516k)","limit":"5h 100%","provider":"claude","title":"Explore"}` — else fall back to `usage_probe.py`; record which surface served in the run record. Verified on herdr 0.8.x, 2026-08-16; the `worktree` command group and the usagebar-absent path are unverified.

**herdr lane placement & native-first rule.** Native-Claude worker units go to the harness's built-in subagents (Agent/Explore tools) first — herdr panes and MCP lanes are for non-native CLIs, interactive/steerable runs, or operator-visible sessions; never burn a pane on work a native subagent covers. New herdr lanes go to their own tab: after `pane split`, run `herdr pane move <pane_id> --new-tab --label <label> --tab-label <text>` (flags verified live 2026-08-17: `--new-tab`, `--label`, `--tab-label`, `--split right|down` all present); probe `herdr pane move --help` once per session before first use as sanity check. `pane move --new-tab` is also the only break-to-tab path — send-keys cannot trigger the UI chord.

**herdr gotchas** (first live judge-panel run 2026-08-17; solutions held rungs 5–8 with zero recurrences):

1. **Wait handles lie both ways.** `agent prompt --wait` / `agent wait` returned `timeout`, `killed`, and `agent_not_running` while judges were done with verdicts written — every single panel. Truth = the verdict FILE + `agent get`; never re-prompt on a wait failure; chain prompt+wait in one background command and re-arm the wait after approvals.
2. **`pane split` requires `--direction right|down`.** Omitting it fails outright.
3. **Permission asks re-fire per path pattern.** Bare enter = Allow-once loop. Send-keys `<label>` tab then enter = Allow always — disposable panes only, record each grant. A thorough judge minting multiple temp dirs re-triggers per new dir: run an approve-loop — poll `agent get`, tab+enter while `blocked`.
4. **Shared-worktree race.** A judge running `git checkout`/`stash` flips HEAD under the other judges mid-gate — produced a false RED, root-caused from reflog. Every judge prompt forbids `checkout`/`stash`/`switch` and mandates `git archive` to a temp dir with PYTHONPATH shadowing; then verify which module actually imported — an editable install can shadow PYTHONPATH.
5. **"End format exactly" gets read literally** — one judge wrote a 53-byte verdict file, evidence only in cropped pane scrollback. Prompt shape: "full audit narrative in the file, ending exactly with …".
6. **Judge brief goes INTO the worktree** as `.judge-brief.md`, with the verdict-file instruction FIRST in the prompt — kills all out-of-root read asks.
7. Misc: `sleep 4` between `pane split` and `agent start` (else `agent_pane_busy`); quote bare `===` in zsh; vault plan filenames don't match branch slugs — `ls | grep` first.
