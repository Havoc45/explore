...
- **Render-before-ask.** Any list or table the user needs in order to answer — a lane's model inventory, the C/I/T table, the Step-6 summary — is rendered in the assistant's **own response text**, never left inside a raw tool result (harness UIs collapse those) and never only *referenced* ("see above"). **Harness reality (live-hit three rounds): response text sandwiched between tool calls in one turn may never be shown** — a render followed by a structured question call in the same turn loses the render. So a render and its question are split **across the turn boundary, never across a tool call**: the response **ends** with the render plus the inviting line — worded exactly: *reply with your picks directly, or reply anything (e.g. "ask") to open the selectors* — if the reply already carries the answer, accept it (fast path); otherwise open the **structured selector as the next turn's first action**, directly under the render in the transcript. **Two exceptions auto-open their selector immediately, no invite**: the wizard's *first* question (mode + lanes, right after the detection probes) and its *last* (save / redo / discard) — for these, the dialog's **preview surface carries the render itself** (the detection table on the mode options; the full roster on the save option), because it is the only surface guaranteed visible in the same turn; the response-window copy still lands in the turn's final text as the durable record. Self-contained questions (billing, consent) use the structured tool directly; previews *supplement* renders everywhere else, never substitute. Plain-text numbered questions remain the no-structured-tool fallback.
...
**Completion criterion:** a detection table in which **every row reads found / absent / unknown** — carried on the auto-opened first question's preview surface (render-before-ask's first-question exception), and landed in the turn's final response text as the durable record.
...
The **save / redo a step / discard** selector auto-opens immediately after the C/I/T confirmation — no invite (render-before-ask's last-question exception). The full setup — host, mode, lanes with billing, the complete roster table with provenance, and the config path that will be written — is carried on the **save option's preview** so the user decides against the roster they can actually see inside the dialog; the same summary lands in the turn's final response text as the durable record.
