...
- **Render-before-ask.** Any list or table the user needs in order to answer — a lane's model inventory, the C/I/T table, the Step-6 summary — is rendered in the assistant's **own response text**, never left inside a raw tool result (harness UIs collapse those) and never only *referenced* ("see above"). **Harness reality (live-hit three rounds): response text sandwiched between tool calls in one turn may never be shown** — a render followed by a structured question call in the same turn loses the render. So a render and its question must share one visible surface: any question that **depends on a render is asked in plain text at the end of the same response, directly under its render**, and the user's reply is read as the answer. The structured question tool is reserved for **self-contained** questions — mode, billing, consent, and confirmations whose full content fits in the options; its preview surface may *supplement* a render, never substitute for it.
...
**Render one model tree for all enabled lanes, then ask under it — in the same response, in plain text.** Model selection depends on the tree, so it follows the render-before-ask plain-text form ("Question mechanics"): one response that ends with the tree — one branch per enabled lane, its available models as leaves: the native lane's aliases; codex's known-good set (the `model` key just read from `~/.codex/config.toml` plus the shipped-roster codex ids — there is no inventory subcommand to list more); opencode's inventory per the long-list rule below — followed directly by one numbered plain-text question per lane ("pick any set: numbers and/or ids, comma-separated; defaults marked"). The user's single reply carries every lane's candidate set. Never split the tree and its questions across a tool-call boundary.
...
After any path: one response that ends with the rendered table — **model / lane / C / I / T / provenance**, with `null` axes rendered `—` — followed directly by the plain-text question **"confirm, or adjust? (name row + axis + value to adjust)"** under it, per render-before-ask. Adjust loops **per value**, not per wizard, re-rendering the table after each change.
...
One response that ends with the whole setup rendered as response text — host, mode, lanes with billing, the full roster table, provenance, and the config path that will be written — followed directly by the plain-text question **"save / redo a step / discard?"** under it (render-before-ask: the user confirms against the roster they can actually see; never ask this through the structured tool in the same turn as the render).
