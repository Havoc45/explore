...
- **Render-before-ask.** Any list or table the user needs in order to answer — a lane's model inventory, the C/I/T table, the Step-6 summary — is rendered in the assistant's **own response text**, never left inside a raw tool result (harness UIs collapse those) and never only *referenced* ("see above"). **Harness reality (live-hit three rounds): response text sandwiched between tool calls in one turn may never be shown** — a render followed by a structured question call in the same turn loses the render. So a render and its question are split **across the turn boundary, never across a tool call**: the response **ends** with the render plus the inviting line — worded exactly: *reply with your picks directly, or reply anything (e.g. "ask") to open the selectors* — if the reply already carries the answer, accept it (fast path); otherwise open the **structured selector as the next turn's first action**, directly under the render in the transcript. **Two exceptions auto-open their selector immediately, no invite**: the wizard's *first* question (mode + lanes) and its *last* (save / redo / discard) — for these, render first in response text (the detection table; the full setup summary), then call the selector **directly after the render in the same turn**, nothing in between. The dialog lands under its render with no user prompt spent. Previews may carry a compact copy of the render as a supplement. Self-contained questions (billing, consent) use the structured tool directly. Plain-text numbered questions remain the no-structured-tool fallback.
...
**Completion criterion:** a detection table in which **every row reads found / absent / unknown**, rendered in response text with the first selector (mode + lanes) auto-opening directly under it in the same turn — no user prompt in between.
...
Render the full setup in response text — host, mode, lanes with billing, the complete roster table with provenance, and the config path that will be written — then the **save / redo a step / discard** selector auto-opens directly under it in the same turn, no invite (render-before-ask's last-question exception). The save option's preview may carry a compact copy of the summary as a supplement; the user decides against the roster rendered above the dialog.
