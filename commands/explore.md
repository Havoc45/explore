...
argument-hint: "[--depth=quick|deep] [--focus=area] [--improve] [--security] [--plan-once \"desc\"] [--review=file] [--execute-level=high plan] [--reconcile] [--init] [--sub-continuous] [--reference=path] [--code-mode=no] [--branch=name] [--bypass-pr-create=yes] [--verbosity=low|medium|high] [--caveman=ultra] [--model=...]"
---
...
- Modifier flags: `--depth=<standard|quick|deep>` (default standard), `--verbosity=<low|medium|high>` (default high, governs ADR/plan wording), `--caveman[=<lite|full|ultra|wenyan-…>]` (compress subagent communication; human output stays at --verbosity), `--model=<model|plan:model,…>` (assign models; default = orchestrator auto-selects best-fit per plan; on Claude Code use Claude models), `--focus=<area>` (scope exploration; a plan-file argument routes to --review), `--sub-continuous[=<handle>|new]` (budget-aware resumable mode — read `${CLAUDE_PLUGIN_ROOT}/skills/explore/references/sub-continuous.md`), `--reference=<path>[,<path>…]` (ingest the maintainer's own docs/notes as ground truth during recon; repeatable), `--code-mode=<yes|no>` (default yes = code CLI/harness, full lifecycle incl. execution; no = chat, planning only — write the ADRs and plans, never execute or touch git), `--branch=<name>` (code mode only — working branch for execution, checked out if it exists, created if not), `--bypass-pr-create=<yes|no>` (default no; code mode only — when yes, push the branch and open a PR after an approved `--improve` diff; never merges), `--issues` (code mode only — publish plans as GitHub issues; public-repo check first).
