...
4. **Return.** Start your reply with `[glm-5.2]` (the UI shows this wrapper's model; the label is the only truth about who worked). Then the final text verbatim, plus `session_id` on its own last line for REVISE rounds. On error, the exact error text.
