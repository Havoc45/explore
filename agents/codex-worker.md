...
- **Chunked briefs.** A brief projected past ~10 minutes should arrive as sections the orchestrator continues on one session (`codex exec resume`); dispatch the section you were given and return its report. A brief that is obviously a monolithic multi-hour job in one prompt: return one line asking the orchestrator to chunk it per delegation.md "Chunked dispatch" instead of dispatching it whole.
